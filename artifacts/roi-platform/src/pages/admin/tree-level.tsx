...
export default function AdminTreeLevel() {
  ...
  const { data, isLoading, isError } = useAdminGetUserBinaryTree(
    selectedUserId,
    { maxDepth },
    {
      query: {
        ...getAdminGetUserBinaryTreeQueryOptions(selectedUserId, { maxDepth }),
  ...
  const leftCount = selectedUser?.directLeft?.length ?? 0;
  const rightCount = selectedUser?.directRight?.length ?? 0;
  ...
  return (
    <AppLayout isAdmin>
      <div className="mb-8 flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground mt-1 max-w-2xl">
            Left/right leg placement for Smart Binary. For sponsor-line genealogy (investment plans), use{" "}
            <strong className="text-foreground">Investment team tree</strong> in the menu.
          </p>
        </div>
      </div>
      ...
      <Card className="p-4 md:p-6 mb-4 space-y-4">
        ...
        {selectedUser ? (
          <p className="text-sm text-muted-foreground">
            Directs:{" "}
            <span className="text-emerald-600 dark:text-emerald-400 font-medium">{leftCount} left</span>
            {" · "}
            <span className="text-sky-600 dark:text-sky-400 font-medium">{rightCount} right</span>
          </p>
        ) : null}
      </Card>
      ...
      <Card className="p-4 md:p-6">
        {!selectedUserId ? (
          <div className="py-12 text-center text-muted-foreground">Select a user to load their tree.</div>
        ) : isLoading ? (
          ...
          <div className="space-y-4">
            ...
            <p className="text-xs text-muted-foreground text-center">
              Showing up to <strong className="text-foreground">{data?.maxDepth ?? maxDepth}</strong> levels below{" "}
              {selectedUser?.name ?? "this member"}.
            </p>
          </div>
        )}
      </Card>
